...
class FlxAnimateFrames extends FlxAtlasFrames
{
	...
	{
        if (Std.is(Path, String) && !Assets.exists(Path))
			return null;

		var data:Access = new Access((Std.is(Path, String)) ? Xml.parse(Assets.getText(Path)).firstElement() : Path.firstElement());
        if (Image == null)
        {
            if (Std.is(Path, String))
            {
    ...
    public static function fromJson(Path:FlxJson, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        if (Std.is(Path, String) && !Assets.exists(Path))
            return null;
        var data:JsonNormal = (Std.is(Path, String)) ? haxe.Json.parse(Assets.getText(Path)) : Path;
        if (Image == null)
        {
            if (Std.is(Path, String))
            {
    ...
    public static function fromStarling(Path:FlxPropertyList, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        if (Std.is(Path, String) && !Assets.exists(Path))
            return null;
        var data:Plist = (Std.is(Path, String)) ? PropertyList.parse(Assets.getText(Path)) : Path;
        if (Image == null)
        {
            if (Std.is(Path, String))
            {
