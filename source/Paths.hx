...
class Paths
{
	...
	{
		var path = getPath('stages/$curStage/images/$key.png', IMAGE, library);
		if (FileSystem.exists(path))
		{
			if (!currentTrackedAssets.exists(key))
			{
	...
	inline static public function getStageSparrowAtlas(curStage:String, key:String, ?library:String)
	{
		...
		return (FlxAtlasFrames.fromSparrow(graphic, File.getContent(file('stages/$curStage/images/$key.xml', library))));
	}
	...
	{
		return (FlxAtlasFrames.fromSpriteSheetPacker(stageImage(key, library), file('stages/$curStage/images/$key.txt', library)));
	}
}
