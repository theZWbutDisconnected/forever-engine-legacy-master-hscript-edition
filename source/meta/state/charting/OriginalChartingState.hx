...
class OriginalChartingState extends MusicBeatState
{
	...
	var curRenderedEventInfos:FlxTypedGroup<FlxText>;
	...
	var value1Typing:FlxInputText;
	var value2Typing:FlxInputText;
	/*
	 * WILL BE THE CURRENT / LAST PLACED NOTE
	**/
	var curSelectedNote:Array<Dynamic>;

	var noteEventName:FlxText;
	var noteEventValue1:FlxText;
	var noteEventValue2:FlxText;
	...
	override function create()
	{
		...
		curRenderedEventInfos = new FlxTypedGroup<FlxText>();
	...
	function addEventsUI():Void
	{
		...
		tab_group_event.add(text);
		var value1InputText = new FlxUIInputText(20, 110, 100, value1);
		value1Typing = value1InputText;
		tab_group_event.add(value1InputText);

		var text:FlxText = new FlxText(20, 130, 0, "Value 2:");
		tab_group_event.add(text);
		var value2InputText = new FlxUIInputText(20, 150, 100, value2);
		value2Typing = value2InputText;
	...
	function addSongUI():Void
	{
		...
		Character.instance.characterArray();
		var characters:Array<String> = Character.instance.characterList;
		Stage.instance.stageArray();
		var stages:Array<String> = Stage.instance.stageList;
	...
	override function update(elapsed:Float)
	{
		...
		value1 = value1Typing.text;
		value2 = value2Typing.text;
	...
	function updateGrid():Void
	{
		...
		while (curRenderedEventInfos.members.length > 0)
		{
			curRenderedEventInfos.remove(curRenderedEventInfos.members[0], true);
		}
		...
		for (i in sectionInfo)
		{
			...
			if (daNoteInfo == -1)
			{
				...
				noteEventValue1.setFormat(Paths.font("vcr.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				noteEventValue1.x -= noteEventValue1.width;
				noteEventValue2 = new FlxText(note.x - GRID_SIZE / 2, note.y + 24, 720, "Value2: " + i[4]);
				noteEventValue2.setFormat(Paths.font("vcr.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				noteEventValue2.x -= noteEventValue2.width;
				curRenderedEventInfos.add(noteEventName);
				curRenderedEventInfos.add(noteEventValue1);
				curRenderedEventInfos.add(noteEventValue2);
			}
		...
		if (data >= -1)
		{
			for (i in _song.notes[curSection].sectionNotes)
			{
				if (i[0] == note.strumTime && i[1] == data)
				{
					...
					for (ii in curRenderedEventInfos.members)
					{
						curRenderedEventInfos.remove(ii);
					}
					break;
				}
			}
	...
	private function addNote():Void
	{
		...
		if (noteData != -1)
			...
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, eventName, value1, value2]);
